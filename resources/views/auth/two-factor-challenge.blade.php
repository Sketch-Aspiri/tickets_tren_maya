<x-guest-layout>
    <div x-data="twoFactorChallenge" data-recovery="{{ $errors->has('recovery_code') ? '1' : '0' }}">
        ...
        <form method="POST" action="{{ route('two-factor.verify') }}">
            ...
            <div x-show="recovery" x-cloak>
                ...
                <x-text-input id="recovery_code" class="mt-1 block w-full" type="text" name="recovery_code" autocomplete="off" x-bind:disabled="usingCode" />
                <x-input-error :messages="$errors->get('recovery_code')" class="mt-2" />
            </div>

            <div class="mt-4 flex flex-col-reverse items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
                <x-text-link x-on:click="toggle">
                    <span x-show="usingCode">{{ __('two_factor.challenge.use_recovery') }}</span>
