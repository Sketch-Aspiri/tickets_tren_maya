<x-app-layout>
    <x-slot name="title">{{ __('tickets.title') }}</x-slot>
    <x-slot name="header">
    ...
    <x-card>
        <form method="GET" action="{{ route('tickets.index') }}" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="sm:col-span-2">
                <x-input-label for="q" :value="__('tickets.filters.search')" />
                <x-text-input id="q" name="q" type="search" class="mt-1 block w-full" :value="$filters['q'] ?? ''" maxlength="100" />
            </div>

            <div>
                <x-input-label for="status" :value="__('tickets.filters.status')" />
            ...
            <div>
                <x-input-label for="priority" :value="__('tickets.filters.priority')" />
            ...
            <div>
                <x-input-label for="category_id" :value="__('tickets.filters.category')" />
            ...
            @if ($teams->isNotEmpty())
                <div>
                    <x-input-label for="team_id" :value="__('tickets.filters.team')" />
            ...
            @if ($responsibles->isNotEmpty())
                <div>
                    <x-input-label for="responsible_id" :value="__('tickets.filters.responsible')" />
            ...
            <div class="flex flex-col justify-end sm:col-span-2 lg:col-span-2">
                <label for="overdue" class="flex min-h-[44px] items-center gap-3 text-sm text-gray-800">
                    ...
                    {{ __('tickets.filters.overdue') }}
                </label>
                <label for="unassigned" class="flex min-h-[44px] items-center gap-3 text-sm text-gray-800">
                    <input id="unassigned" name="unassigned" type="checkbox" value="1" class="h-5 w-5 rounded border-gray-500 text-brand-green focus:ring-2 focus:ring-brand-teal" @checked($filters['unassigned'] ?? false)>
                    {{ __('tickets.filters.unassigned') }}
                </label>
            </div>

            <div class="flex items-end gap-2 sm:col-span-2 lg:col-span-4">
                <x-primary-button>{{ __('common.actions.filter') }}</x-primary-button>
                <x-text-link :href="route('tickets.index')">{{ __('common.actions.clear') }}</x-text-link>
            </div>
        </form>
    </x-card>
